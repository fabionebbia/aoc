defmodule Day16.Evaluator do
  ...
  def eval({:sum, _, _, values}), do: eval_then(values, &Enum.sum/1)

  def eval({:product, _, _, values}), do: eval_then(values, &Enum.product/1)

  def eval({:min, _, _, values}), do: eval_then(values, &Enum.min/1)

  def eval({:max, _, _, values}), do: eval_then(values, &Enum.max/1)
